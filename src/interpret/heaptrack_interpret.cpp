...
private:
    ...
    void initializePaths()
    {
        ...
        for (const auto& p : m_extraLibPaths) {
            path += p + ":";
        }
    ...
    std::vector<std::string> m_extraLibPaths;
...
int main(int argc, char** argv)
{
    ...
    std::string sysroot;
    std::vector<std::string> debugPaths, extraLibPaths;

    if (vm.count("sysroot")) {
        sysroot = vm["sysroot"].as<std::string>();
    }
    if (vm.count("debugPaths")) {
        debugPaths = vm["debugPaths"].as<std::vector<std::string>>();
    }
    if (vm.count("extraLibPaths")) {
        extraLibPaths = vm["extraLibPaths"].as<std::vector<std::string>>();
    }
    ...
    AccumulatedTraceData data(sysroot, debugPaths, extraLibPaths);
